    ...
    if (!document.getElementById('userMenu')) {
        ...
        userMenu.innerHTML = `
            <div class="user-info">
                ...
                <span class="user-name">${appState.user.fullName}</span>
            </div>
            <button class="btn btn-sm btn-outline" id="btnLogout">
...
function initializeApp() {
    ...
    document.getElementById('btnAdminView').addEventListener('click', () => {
        // Verificar autenticación antes de cambiar a vista admin
        if (!appState.isAuthenticated) {
            if (confirm('Necesitas iniciar sesión para acceder al panel de administración. ¿Deseas ir a la página de login?')) {
                window.location.href = '/login';
            }
            return;
        }
...
}, 10000); // Refresh cada 10 segundoss